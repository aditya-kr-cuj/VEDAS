...
export async function performanceTrendHandler(req: Request, res: Response): Promise<void> {
  const tenantId = req.tenantId;
  const userId = req.auth?.userId;
  if (!tenantId) throw new HttpError(400, 'Tenant context is required');
  if (req.role === 'student') {
  ...
  const result = await getPerformanceTrend({ tenantId, studentId: req.params.id });
  res.status(200).json(result);
}

export async function batchComparisonHandler(req: Request, res: Response): Promise<void> {
  const tenantId = req.tenantId;
  if (!tenantId) throw new HttpError(400, 'Tenant context is required');
  const result = await getBatchComparison({ tenantId, batchId: req.params.id });
  res.status(200).json({ comparisons: result });
}

export async function testComparisonHandler(req: Request, res: Response): Promise<void> {
  const tenantId = req.tenantId;
  if (!tenantId) throw new HttpError(400, 'Tenant context is required');
