  ...
  const fetchLogs = useCallback(
    async (page = 1) => {
      ...
      try {
        ...
        const res = await api.get("/notifications/sms/logs", { params });
        setLogs(res.data.logs ?? []);
        setPagination(
          res.data.pagination ?? { page: 1, limit: 25, total: 0, totalPages: 0 }
        );
      } catch {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white">SMS Logs</h2>
          <p className="mt-1 text-sm text-slate-400">
            Monitor all outgoing SMS messages from your institute
          </p>
        </div>
        <div className="flex items-center gap-2 text-xs text-slate-500">
          <span className="inline-block h-2 w-2 rounded-full bg-emerald-400" />{" "}
          Sent
          <span className="ml-2 inline-block h-2 w-2 rounded-full bg-red-400" />{" "}
          Failed
            ...
            <Table>
              <THead>
                <TR className="border-white/5 hover:bg-transparent">
                  ...
                  <TH className="text-slate-400">Message</TH>
                  <TH className="text-slate-400">Status</TH>
                  <TH className="text-slate-400">Credits</TH>
              ...
              <TBody>
                {logs.map((log) => (
                  ...
                  >
                    <TD className="whitespace-nowrap text-sm font-mono text-slate-200">
                      {log.recipient_phone}
                    </TD>
                    <TD className="max-w-[240px] truncate text-sm text-slate-300">
                      {log.message}
                    </TD>
                    <TD>
                      <span
